...
fn check_output_dir_exists(config: &Config) -> Result<()> {
  let output_dir = std::path::Path::new(&config.bib_output)
    .parent()
    .ok_or_else(|| {
      anyhow::anyhow!("Impossible de déterminer le répertoire de sortie")
    })?;
...
fn convert_to_zotero_item(raw_item: &Value) -> Result<ZoteroItem> {
  let id = gen_id(raw_item).ok_or_else(|| {
    anyhow::anyhow!("Impossible de générer l'ID pour l'entrée")
  })?;
...
fn validate_and_convert_items(
  raw_items: &[Value],
) -> (Vec<ZoteroItem>, Vec<String>) {
